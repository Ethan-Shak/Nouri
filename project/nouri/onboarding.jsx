/* NOURI — Onboarding flow: the gate before the main tabs.
   Welcome → Verify (phone/email → OTP) → Situation (persona template) →
   Profile (name · allergies · targets) → Connect → Payment → Ready. */

const { useState: _useState, useEffect: _useEffect, useRef: _useRef, useMemo: _useMemo } = React;

const OB_CORE = ['verify', 'situation', 'profile', 'connect', 'payment']; // progress-tracked
...
function OtpStep({ destination, otpStyle, onBack, onVerified, onChangeDest }) {
  ...
  const [status, setStatus] = _useState('idle'); // idle | verifying | ok | error
  ...
  // single guarded verification run — not driven by an effect dep so the
  // timers can't be cancelled by the status flip mid-flight
  const runVerify = () => {
...
function ReadyStep({ persona, profile, payment, autonomy, onEnter }) {
  const [phase, setPhase] = _useState('calibrating'); // calibrating | ready
...
/* ════════════════════ FLOW CONTROLLER ════════════════════ */
function OnboardingFlow({ welcomeStyle = 'features', otpStyle = 'boxed', channelMode = 'choice',
                          onComplete }) {
  ...
  // keep channel in sync if the tweak forces a mode
  _useEffect(() => {
  ...
  // when a persona is chosen, prefill profile + connectors from its template
