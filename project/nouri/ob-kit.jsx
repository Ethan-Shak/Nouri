/* NOURI — Onboarding UI kit: setup-assistant chrome + form primitives.
   Apple-setup feel (big serif title, supporting body, pinned primary button,
   thin step progress) rendered in NOURI's bone/sage/clay system. */

const { useState: _obUseState, useEffect: _obUseEffect, useRef: _obUseRef } = React;
...
function ObFrame({ progress, total, onBack, children, footer, animKey }) {
  return (
    <div style={{
...
function ObHero({ icon, tone = 'sage', size = 76, children }) {
...
function ObFeature({ icon, title, desc, tone = 'sage' }) {
  const tint = tone === 'clay' ? 'var(--clay-soft)' : 'var(--sage-soft)';
  const fg = tone === 'clay' ? 'var(--clay)' : 'var(--sage)';
  return (
...
/* ───────────── Text field ───────────── */
function ObField({ value, onChange, placeholder, type = 'text', inputMode, maxLength,
...
/* ───────────── Segmented control ───────────── */
function ObSegment({ value, options, onChange }) {
  return (
    <div style={{
...
/* ───────────── OTP code input ───────────── */
function ObOtp({ length = 6, value, onChange, variant = 'boxed', autoFocus, status }) {
...
/* ───────────── Stepper (− value +) ───────────── */
function ObStepper({ label, value, unit = '', min = 0, max = 9999, step = 1, onChange }) {
  const btn = (dir) => (
...
function ObSelectCard({ selected, onClick, icon, tone = 'sage', title, sub, tag }) {
  const tint = tone === 'clay' ? 'var(--clay-soft)' : 'var(--sage-soft)';
  const fg = tone === 'clay' ? 'var(--clay)' : 'var(--sage)';
  return (
...
/* allergen catalogue (mirrors the in-app Allergies screen) */
const OB_ALLERGENS = [
...
/* persona → glyph + accent for the situation step */
const OB_PERSONA_META = {
...
Object.assign(window, {
  ...
  ObProgress, ObFrame, ObHero, ObFeature, ObField, ObSegment, ObOtp,
