/* NOURI — App entry: tabs, sheet, tweaks panel, scaling. */
...
/* ───────────────────── App ───────────────────── */
function App() {
  const tweakDefaults = window.NOURI_TWEAK_DEFAULTS || { persona: 'quarantined', autonomy: 3, hour: 12, theme: 'light' };
  const [t, setTweak] = useTweaks(tweakDefaults);

  const [tab, setTab] = useState('today');
  const [sheet, setSheet] = useState(null); // { meal, initialStage }
  const [subpage, setSubpage] = useState(null); // 'allergies' | 'spend'

  // onboarding gate — runs before the main tabs are reachable
  const [view, setView] = useState('onboarding'); // 'onboarding' | 'app'
  const [obKey, setObKey] = useState(0); // bump to remount/reset the flow
    ...
    <>
      <div style={{
        ...
        <IOSDevice width={PHONE_W} height={PHONE_H} dark={dark}>
          ...
          /* full-bleed content: status-bar safe, scrolling middle, fixed bottom nav */
          <div style={{
            background: 'var(--bg)', color: 'var(--ink)',
            ...
            <div style={{ paddingTop: 58, flex: '0 0 auto' }}>
              <TopBar persona={persona} autonomy={autonomy}/>
            </div>

            <div data-screen-label={`01 ${tab}`} className="scroll" style={{
