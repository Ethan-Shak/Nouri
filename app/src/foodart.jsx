...
export function dishPlatform(meal) {
...
export function dishStats(meal) {
...
export function FoodImage({ meal, height = 200, overlay }) {
  const art = useMemo(() => dishArt(meal?.item), [meal?.item]);
  const seed = useMemo(() => _hash((meal?.item || '') + (meal?.from || '')), [meal]);
  const rng = useMemo(() => _rng(seed), [seed]);
...
export function PlatformCard({ meal }) {
  ...
  return (
    <div style={{
      ...
      <div style={{
        ...
        <span className="mono" style={{ fontSize: 10, color: 'var(--muted)', letterSpacing: 0.6 }}>
          {stats.orderId}
        </span>
      </div>
      ...
      <div style={{ display: 'flex', gap: 12, alignItems: 'center', padding: '12px 14px' }}>
        ...
        <div style={{ flex: 1, minWidth: 0 }}>
          ...
          <div style={{ display: 'flex', alignItems: 'center', gap: 8, fontSize: 11, color: 'var(--muted)', marginTop: 3 }}>
      ...
      <div style={{ display: 'grid', gridTemplateColumns: 'repeat(3, 1fr)', borderTop: '0.5px solid var(--hairline-2)' }}>
        {[
          ...
          <div key={s.l} style={{
            ...
            <div className="mono" style={{ fontSize: 9, color: 'var(--muted)', textTransform: 'uppercase', letterSpacing: 0.6, marginTop: 2 }}>{s.l}</div>
          </div>
        ))}
      </div>
      ...
      <div style={{
        ...
        <span style={{ fontSize: 11, color: 'var(--ink-2)' }}>Placed by NOURI · via browser agent</span>
      </div>
    </div>
  );
}
